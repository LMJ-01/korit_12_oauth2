...
2. Box
    - `<div>`와 거의 유사한 역할인데, MUI 기반 CSS를 JS 객체 형태로 넣을 수 있다는 특지징이 있습니다.
    - 이상의 코드에서는 다수의 Typography 컴포넌트들을 통쨰로 묶어서 마진 등을 조절할 떄 사용했습니다.

3. Avatar
    - 사용자 프로필 사진이나 이름 첫 문자를 동그랗게 뜨워줄 떄  사용합니다.
...
라는 전체 url을 rturn하게 되는데, 그 중에서 `?` 뒷 부분에 있는 부분이 key-value형태와 유사하다는 것을 알 수 있습니다. token, email, name, role이라는 key를 갖고, `=` 뒷 부분이 value가 되며, 다음 key로 연결될 떄 `&`를 쓴 형태네요. 백엔드에서는 `@PathVariable`을 통해서 endpoint 뒷부분의 id를 동적으로 가져올 수 있었습니다. `http://localhost:8080/api/vehicles/1` 과에서 `/1` 부분이었죠. 근데 search 라고 하는 엔드포인트를 만들어본 적도 있습니다. repository 내에서 커스텀 메서드를 만들었을 떄 였습니다. 그때 저희는 물음표 부분을 확인할 수 있었는데, 이처럼 `?` 뒷부분을 쿼리스트링(query string)이라고 칭합니다.
...
- `{replace : true}` : OAuth2 로그인이 완료된 다음에 `/`를 통해 메인페이지로 진입하게 될텐데, 그떄 뒤로가기 버튼 누르면 다시 callback page가 나오게 될겁니다. 그러면 불필요한 fetch가 일어날 수 있겠네요. 그거 막아주는 앱니다.